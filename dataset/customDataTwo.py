...
class customData(Dataset):
    def __init__(self, img_path, txt_path, phase = '',data_transforms=None, loader = default_loader):
        ...
        self.img_name = []
        self.img_label = []
        endOfMetaTrain = int(len(img_paths)*0.7)
        endOfMetaValid = int(len(img_paths)*0.8)
        endOfMetaTest = int(len(img_paths)*0.9)
        ...
        #             }
        if phase == 'meta_train':
            phase_img_paths = img_paths[:endOfMetaTrain]
        elif phase == 'meta_val':
            phase_img_paths = img_paths[endOfMetaTrain:endOfMetaValid]
        elif phase == 'meta_test':
            phase_img_paths = img_paths[endOfMetaValid:endOfMetaTest]
        elif phase == 'test':
            phase_img_paths = img_paths[endOfMetaTest:]

        for path in phase_img_paths:#不能先吃
        ...
        if phase =='meta_train':
            ...
            for path in img_paths[lengthOfTrain:endOfVal]:
                if "R_0402" in path:
                    ok_ng = path.split('/')[-2]#只保留類別名稱
            ...
            for path in img_paths[endOfVal:]:
                if "R_0402" in path:
                    ok_ng = path.split('/')[-2]#只保留類別名稱
